...
public class ThreadTest02 {
	...
	public static void main(String[] args) {
		...
		//     이 인스턴스를 Thread클래스의 인스턴스를 생성할 때 생성자의 인자값으로 넣어 생성한다.
		//     이 때 생성된 Thread클래스의 인스턴스의 start()메소드를 호출해서 실행한다.
		MyRunner1 r = new MyRunner1();
		Thread th2 = new Thread(r);
		th2.start();
		
		// 방법2-1
		// ==> Runnable인터페이스의 익명구현체를 이용하는 방법
			...
			public void run() {
				...
				for (int i = 1; i <= 200; i++) {
					...
					try {
						...
						// '시간'은밀리세컨드 단위를 사용한다. 즉, 1초는 1000으로 나타낸다.
						Thread.sleep(100);
...
class MyThread1 extends Thread{
	...
	@Override
	public void run() {
		// 이 run()메소드에는 이 스레드가 처리할 내용을 작성하면 된다.
		// 예시 '*'문자 200개 출력

 ...
 *  
 *  - Comparable은 Collection에 추가되는 데이터 자체에 정렬기준을 넣고 싶을 때 구현하는 인터페이스이다.
 *    (내부 정렬 기준 이라고 한다.)
 *  
 *  - Comparator은 외부에 별도로 정렬 기준을 구현하고 싶을 때 사용하는 인터페이스이다.
 *    (외부 정렬 기준 이라고 한다.)
 *  
 *  - Comparable에서는 comparaTo()메소드를 재정의 해야 하고,
 *    Comparator에서는 compara()메소드를 재정의 해야 한다.
 *    
 *  - String클래스, Wrapper클래스, Date클래스, File클래스에는 내부 정렬 기준이 이미 구현되어 있음.
...
public class ListSortTest01 {
	...
	public static void main(String[] args) {
		...
		//정렬 작업은 Collections.sort()메소드를 이용하여 정렬한다.
...
class Desc implements Comparator<String>{
	...
	// 예) 오름차순일 경우 ==> 앞의 값이 크면 양수, 같으면 0, 뒤의값이 크면 음수를 반환하도록 구현하면 된다.
	@Override
